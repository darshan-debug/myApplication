import javax.swing.*;//will act like a server(shld be runned ,before sender code)
...
public class receiver {
    ...
    receiver(){
        ...
        ack.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e5){
                try{dos.writeInt(expectedSeqNo-1);dlm.addElement("ack- "+(expectedSeqNo-1)+" sent");}catch(Exception ew){dlm.addElement("error:sending ack- "+(expectedSeqNo-1));}
        ...
        for(int z=0;z<20;z++){packett[z]=new JLabel(" "+z+" ");packett[z].setOpaque(true);packett[z].setBackground(Color.WHITE);}packett[0].setForeground(Color.red);
        //creating pane,to display summary
        dlm=new DefaultListModel<String>();
        jl=new JList<String>(dlm);jl.setLayoutOrientation(JList.VERTICAL);jl.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        jl.setVisibleRowCount(20);
        JScrollPane scrollArea=new JScrollPane(jl);scrollArea.setSize(1000, 600);
        vertical=scrollArea.getVerticalScrollBar();
        docTimer=new Timer(700,new ActionListener(){
            public void actionPerformed(ActionEvent e19){vertical.validate();/*validation is,updating "Maximum" of vertical*/vertical.setValue( vertical.getMaximum() );}
        });
        ...
        buttonPane.add(ack);buttonPane.add(Box.createHorizontalStrut(120));buttonPane.add(d_ack);
    ...
    public static void afterReceivingPacket(){
        int w;
        try{w=dis.readInt();
            if(w==expectedSeqNo){dlm.addElement("packet no: "+w+" received");expectedSeqNo++;packett[expectedSeqNo-1].setForeground(Color.black);packett[expectedSeqNo-1].setBackground(Color.GREEN);if(expectedSeqNo<20){packett[expectedSeqNo].setForeground(Color.red);}} 
    ...
    public static void resetApplication(){
        dlm.clear();dlm.addElement("Listening at port 4040"); dlm.addElement("TCP connection estabilished!!");
        ack.setEnabled(false);d_ack.setEnabled(false);expectedSeqNo=0;
        for(int z=0;z<20;z++){packett[z].setVisible(false);packett[z].setBackground(Color.WHITE);packett[z].setForeground(Color.black);packett[z].setVisible(true);}packett[0].setVisible(false);packett[0].setForeground(Color.red);packett[0].setVisible(true);
